...
This vignette documents functions developed for the research of Endelman et al. (2024) "Targeted genotyping-by-sequencing of potato and software for imputation" (link coming soon). Please cite this work if you use the software.
...
The above code illustrates use of the `gbs` function, which replaces the DArT genotype calls with calls based on the R/updog software ([Gerard et al. 2018](https://doi.org/10.1534/genetics.118.301468)), using the "norm" prior. R/updog can account for several departures from the traditional binomial model, including allelic bias and overdispersion, and these parameters are stored in the VCF. 85 samples is potentially too small for robust genotype calling, but in practice you will have at least 384 DArTag samples, since that is the minimum submission.
...
As shown above, the `GT2DS` function in polyBreedR can be used to convert VCF GT character strings to integer allele dosages (DS). The code below compares the genotype calls between DArT (data1) updog (data2) for the first marker, which were identical except for one sample.
...
Two functions were created in polyBreedR for low-to-high imputation. `impute_L2H` uses the Random Forest method based on a training population of individuals genotyped under both platforms. In general, a larger training population leads to better the imputation accuracy. The default behavior is to use the 100 closest markers and 100 classification trees, but these are adjustable parameters. Consult the help page for more details.

The other function, `impute_PO`, uses the [software PolyOrigin](https://github.com/chaozhi/PolyOrigin.jl) to impute markers in the F1 progeny of a partial diallel population based on linkage analysis. To use `impute_PO`, you will need to install Julia and PolyOrigin and prepare for [command line execution](https://julialang.org/downloads/platform/). The high-density marker file for `impute_PO` contains the phased parental genotypes, which can be generated with PolyOrigin. 
...
impute_PO(ped.file = ped.file, high.file = high.file,
